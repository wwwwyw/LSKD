...
def run_step(opts, world_size, rank, device):
    ...
    if TRAIN:
        ...
        for cur_epoch in range(opts.epochs):
            ...
            initial_task_loss=epoch_loss[-1]
            normalize_coeff = 9 / torch.sum(model.module.head.lossweights.data, dim=0)
            model.module.head.lossweights.data = model.module.head.lossweights.data * normalize_coeff
            logger.info(
            ...
            logger.add_scalar("---Loss", epoch_loss[0] + epoch_loss[1] +epoch_loss[2], cur_epoch)
            # logger.add_scalar("Loss-KD", epoch_loss[1], cur_epoch)
            # logger.add_scalar("Loss-CE", epoch_loss[0], cur_epoch)
            # logger.add_scalar("Loss-pod", epoch_loss[2], cur_epoch)
            # logger.add_scalar("Loss-pod0", epoch_loss[3], cur_epoch)
            # logger.add_scalar("Loss-pod1", epoch_loss[4], cur_epoch)
            # logger.add_scalar("Loss-pod2", epoch_loss[5], cur_epoch)
            # logger.add_scalar("Loss-pod3", epoch_loss[6], cur_epoch)
            # logger.add_scalar("Loss-pod4", epoch_loss[7], cur_epoch)
            # logger.add_scalar("Loss-pod5", epoch_loss[8], cur_epoch)
            # logger.add_scalar("Loss-waiji", epoch_loss[9], cur_epoch)

            # logger.add_scalar("Loss-before", epoch_loss[11] + epoch_loss[10] +epoch_loss[12], cur_epoch)

            # logger.add_scalar("Loss-KD-before", epoch_loss[11], cur_epoch)
            # logger.add_scalar("Loss-CE-before", epoch_loss[10], cur_epoch)
            # logger.add_scalar("Loss-pod-before", epoch_loss[12], cur_epoch)
            # logger.add_scalar("Loss-pod0-before", epoch_loss[13], cur_epoch)
            # logger.add_scalar("Loss-pod1-before", epoch_loss[14], cur_epoch)
            # logger.add_scalar("Loss-pod2-before", epoch_loss[15], cur_epoch)
            # logger.add_scalar("Loss-pod3-before", epoch_loss[16], cur_epoch)
            # logger.add_scalar("Loss-pod4-before", epoch_loss[17], cur_epoch)
            # logger.add_scalar("Loss-pod5-before", epoch_loss[18], cur_epoch)
            # logger.add_scalar("Loss-waiji-before", epoch_loss[19], cur_epoch)
