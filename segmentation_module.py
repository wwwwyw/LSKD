...
def make_model(opts, classes=None):
    ...
    if not opts.no_pretrained:
        pretrained_path = os.path.join(opts.code_directory, f'/home/gmz/wyw/CVPR2021_PLOP-main/pretrained/{opts.backbone}_{opts.norm_act}.pth.tar')
...
class IncrementalSegmentationModule(nn.Module):
    ...
    def get_last_shared_layer(self):
        return self.head.get_last_shared_layer()
    def get_last_shared_layer1(self):
        return self.head.get_last_shared_layer1()        
    def get_layer0(self):
        return self.body.get_layer0()
    def get_layer1(self):
        return self.body.get_layer1()
    def get_layer2(self):
        return self.body.get_layer2()
    def get_layer3(self):
        return self.body.get_layer3()
    ...
    def forward(self, x, scales=None, do_flip=False, ret_intermediate=False, only_bg=False):
        ...
        if ret_intermediate:
        #     large_pre_logits = F.interpolate(
        #     large_pre_logits, size=out_size, mode="bilinear", align_corners=False
        # )
            return sem_logits, {
                "body": out[1],
                "pre_logits": out[2],
